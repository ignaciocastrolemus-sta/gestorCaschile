...
import { ScrollView, View, Text, Pressable, StyleSheet, Alert, Modal } from "react-native";
...
export default function EncuadreRendiciones({ token }) {
  ...
  // Mantiene la consulta base al backend y deja los filtros rápidos en frontend.
  ...
  // Opciones dinámicas para la fila de filtros.
  const opciones = useMemo(() => {
    ...
    items.forEach((it) => {
      periodos.add(getPeriodoLabel(it?.viaje?.fechaInicio));
      capacitadores.add(it?.viaje?.capacitador || "-");
      estados.add(it?.estado || "-");
    });

    return {
      periodos: ["todos", ...Array.from(periodos).sort((a, b) => a.localeCompare(b, "es"))],
  ...
  // Filtrado visual para mantener experiencia rápida.
  const itemsFiltrados = useMemo(() => {
    return items.filter((it) => {
      const periodo = getPeriodoLabel(it?.viaje?.fechaInicio);
      const capacitador = it?.viaje?.capacitador || "-";
      const estado = it?.estado || "-";

      const okPeriodo = filtro.periodo === "todos" || normalize(filtro.periodo) === normalize(periodo);
      const okCapacitador =
      ...
      return okPeriodo && okCapacitador && okEstado;
    });
  }, [items, filtro]);
  ...
  const aplicarFiltros = () => setFiltro({ ...draftFiltro });

  const limpiarFiltros = () => {
    const base = { periodo: "todos", capacitador: "todos", estado: "todos" };
    setDraftFiltro(base);
    setFiltro(base);
  };
  ...
  const descargarCsv = (soloItem) => {
    ...
    const encabezado = [
      ...
      [csvCell("Filtro periodo"), csvCell(filtro.periodo)].join(sep),
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <div class="meta"><b>Filtro periodo:</b> ${esc(filtro.periodo)}</div>
          ...
          <table>
            ...
            <tbody>
              ${rowsHtml || '<tr><td colspan="10">Sin datos para exportar</td></tr>'}
            </tbody>
          </table>
        </body>
      </html>
        ...
        <View style={styles.filterRow}>
          <View style={styles.chipBox}>
            ...
            <View style={styles.selectWrap}>
              <Picker
                selectedValue={draftFiltro.periodo}
                onValueChange={(v) => onChange("periodo", v)}
                style={styles.picker}
              >
      ...
      {itemsFiltrados.length === 0 ? (
        ...
        <Card style={{ padding: 0, overflow: "hidden" }}>
          <View style={styles.tableHeader}>
            <Text style={[styles.th, { flex: 2.3 }]}>Rendicion</Text>
            ...
            <Text style={[styles.th, { flex: 1.6, textAlign: "right" }]}>Acciones</Text>
          </View>
            ...
            return (
              ...
              >
                ...
                <View style={[styles.rowActions, { flex: 1.6 }]}>
                  ...
                  <Pressable style={styles.actionBtn} onPress={() => descargarCsv(it)}>
                    <Text style={styles.actionText}>CSV</Text>
                  </Pressable>
                  <Pressable style={styles.actionBtn} onPress={() => descargarExcel(it)}>
